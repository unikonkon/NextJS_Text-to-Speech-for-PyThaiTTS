...
export default function TextToSpeechForm() {
  ...
  const containsEnglishOrNewline = (input: string): boolean => {
    // Check for English characters (a-z, A-Z) and newlines
    const englishRegex = /[a-zA-Z]/;
    const newlineRegex = /\n/;
    return englishRegex.test(input) || newlineRegex.test(input);
  };

  const handleTextChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    const newText = e.target.value;
    if (containsEnglishOrNewline(newText)) {
      setError('Please use only Thai characters. English characters and newlines are not allowed.');
      return;
    }
    setError(null);
    setText(newText);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (containsEnglishOrNewline(text)) {
      setError('Please use only Thai characters. English characters and newlines are not allowed.');
      return;
    }
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label htmlFor="text" className="block text-sm font-medium text-gray-700 mb-1">
            Enter Thai text to convert to speech
          </label>
          <textarea
            id="text"
            value={text}
